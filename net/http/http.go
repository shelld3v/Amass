...
var (
	...
	crawlRE         = regexp.MustCompile(`\.[a-z]{2,6}($|#|\?)`)
	crawlFileStarts = []string{"htm", "as", "php", "js", "inc"}
	crawlFileEnds   = []string{"html", "do", "action", "cgi", "cfm"}
			...
			processURL := func(u string) {
				if p, err := url.Parse(u); err == nil && whichDomain(p.Hostname(), newScope) != "" {
					...
					if ext := crawlRE.FindString(p.Path); ext != "" {
						...
						var found bool
						for _, t := range crawlFileStarts {
							if strings.HasPrefix(ext, t) {
								found = true
								break
							}
						}
						for _, t := range crawlFileEnds {
							if strings.HasSuffix(ext, t) {
								found = true
								break
							}
						}
